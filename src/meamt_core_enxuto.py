...
def setup_deap_classes(n_obj):
    """Inicializa as classes do DEAP voltadas ao MoeaBench."""
...
def build_toolbox(funcao_avaliacao, ind_size, n_pop, n_obj):
    """Constrói o toolbox do DEAP de forma dinâmica."""
...
# ==========================================
# 2. OPERADORES DO MEA-MT ENXUTO
# ==========================================
def calc_combined_fitness(ind, table_idx):
    """
    Versão Enxuta: Calcula o fitness do indivíduo puramente para o 
    objetivo designado pela tabela.
    """
    obj_index = table_idx - 1 
    return -ind.fitness.wvalues[obj_index]

def gen_inicial_tables(pop_ini, num_tables, table_size):
    """Distribui a população nas tabelas (ND + 1 por Objetivo)."""
    tables = dict()
...
def select_parents(tables, num_tables):
    """Seleciona pais baseado no 'score' das tabelas."""
    selected = []
    for _ in range(2):
...
def insert_in_tables(tables, num_tables, off, max_table_size):
    """Insere o filho nas tabelas. (Removido o parâmetro n_obj na versão enxuta)"""
    # 1. Tabela ND
    tabela_nd = tables[0]
    ...
    # 2. Tabelas isoladas (versão enxuta)
    for i in range(1, num_tables):
        bisect.insort(tables[i], off, key=lambda ind: calc_combined_fitness(ind, i))
...
# 3. LOOP EVOLUTIVO
# ==========================================
def run(tables, num_tables, max_table_size, max_fes, avaliacoes_iniciais, toolbox, cxpb, mutpb, reset):
    """Motor Steady-State do MEA-MT Enxuto."""
    fes_count = avaliacoes_iniciais
    avaliacoes_por_geracao = avaliacoes_iniciais 
    proximo_reset = fes_count + avaliacoes_por_geracao
    geracao_virtual = 1
    ...
    while fes_count < max_fes:
        ...
        # 4. Avaliação (Onde o FES é cobrado)
        ...
        for off in offspring:
             insert_in_tables(tables, num_tables, off, max_table_size)
        
        # 6. Reset do Score das Tabelas
        if fes_count >= proximo_reset:
            if geracao_virtual % reset == 0:
                for t in tables.values():     
                    t.score = 0.0
            
            geracao_virtual += 1
            proximo_reset += avaliacoes_por_geracao